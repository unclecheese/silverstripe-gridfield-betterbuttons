...
class BetterButtonAction_FrontendLinks extends BetterButtonAction 
{
    ...
    public function getButtonLink()
    {
        return $this->request->record->hasMethod('Link') ? $this->request->record->Link() : "";        
    }

    public function shouldDisplay() 
    {
        return $this->request->record && $this->request->record->hasMethod('Link');
    }
...
class BetterButtonAction_PrevNext extends BetterButtonAction 
{
    ...
    public function getButtonHTML()
    {
        ...
        $previousRecordID = $this->request->getPreviousRecordID();
        $cssClass = $previousRecordID ? "cms-panel-link" : "disabled";
        $prevLink = $previousRecordID ? Controller::join_links($this->request->gridField->Link(),"item", $previousRecordID) : "javascript:void(0);";
        ...
        $nextRecordID = $this->request->getNextRecordID();
        $cssClass = $nextRecordID ? "cms-panel-link" : "disabled";
        $prevLink = $nextRecordID ? Controller::join_links($this->request->gridField->Link(),"item", $nextRecordID) : "javascript:void(0);";
